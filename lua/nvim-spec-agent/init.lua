-- Neovim Spec Agent Lua Interface
local M = {}
...
function M.setup(user_config)
  ...
  if not config.rust_binary_path then
    vim.notify('nvim-spec-agent: Rust binary not found. Please run the build script.', vim.log.levels.WARN)
    return
  end
...
function M.start_rust_backend()
  ...
  if state.rust_job_id <= 0 then
    vim.notify('Failed to start nvim-spec-agent backend', vim.log.levels.ERROR)
    return false
  end
  
  state.initialized = true
  vim.notify('nvim-spec-agent backend started', vim.log.levels.INFO)
